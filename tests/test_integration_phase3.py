"""
Integration tests for Phase 3: Nested mutation tracking with proxies (sync).

These tests verify that ProxiedMap and ProxiedList work correctly with
real Firestore operations, including save/fetch round-trips.
"""
...
class TestPhase3NestedMutationTracking:
    """Test nested mutation tracking with ProxiedMap and ProxiedList."""

    @pytest.fixture
    def db(self, client):
        """Create a FireProx instance."""
        return FireProx(client)

    @pytest.fixture
    def collection(self, db, firestore_test_harness):
        """Create a test collection."""
        return db.collection('phase3_users')

    def test_dict_assignment_wraps_in_proxy(self, collection):
    ...
    def test_list_assignment_wraps_in_proxy(self, collection):
    ...
    def test_nested_dict_mutation_marks_dirty(self, collection):
    ...
    def test_nested_list_mutation_marks_dirty(self, collection):
    ...
    def test_nested_mutation_save_round_trip(self, collection):
    ...
    def test_deeply_nested_structures(self, collection):
    ...
    def test_mixed_nested_structures(self, collection):
    ...
    def test_fetch_wraps_in_proxies(self, collection):
    ...
    def test_to_dict_unwraps_proxies(self, collection):
    ...
    def test_invalid_field_name_rejected(self, collection):
    ...
    def test_excessive_nesting_rejected(self, collection):
    ...
    def test_list_of_dicts(self, collection):
    ...
    def test_append_dict_to_list(self, collection):
    ...
    def test_dict_update_method(self, collection):
    ...
    def test_list_extend_method(self, collection):
    ...
    def test_empty_dict_and_list(self, collection):
    ...
    def test_none_and_primitives_not_wrapped(self, collection):
        """Test that None and primitives are not wrapped."""
        user = collection.new()
        user.none_value = None
        user.string_value = 'hello'
        user.int_value = 42
        user.bool_value = True
        user.save()

        # Should not be wrapped
        assert user.none_value is None
        assert user.string_value == 'hello'
        assert user.int_value == 42
        assert user.bool_value is True

    def test_conservative_save_whole_field(self, collection):
